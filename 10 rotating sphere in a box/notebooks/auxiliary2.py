...
#     cbar.set_label(label='$|\\mathbf{u}| ~ \\left(10^{-3}~\\mathrm{mm}/\\mathrm{s}\\right)$', fontsize=16)
    ...
    cbar.set_ticklabels(['$-10^{-2}$', '$0$','$10^{-2}$'])
...
def particleTrjOverview(df):
    ...
    axes[0].plot(df.time, df.position_x, label='$x - x_0$')
    axes[0].plot(df.time, df.position_y + 40.5, label='$y - y_0$')
    # axes[0].set_ylabel("$q$")
