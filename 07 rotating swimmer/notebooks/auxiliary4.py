...
# ------------------------------------------ methods ------------------------------------------
# ---------------------------------------------------------------------------------------------
def fluidOverview(bigDf, title):
    df = bigDf.query("coordinate_x**2 <= 0.1**2")
...
#     fig.suptitle(f"$t = {df.time.values[0]:.2f}$", fontsize=16)
    ...
    axes[0].pcolormesh(X, Y,
        np.sqrt(df.fluidVelocity_y**2 + df.fluidVelocity_z**2).unstack().values.transpose(),
        vmin=lowerLim,
        vmax=upperLim,
...
def particleTrjOverview(df):
    ...
    axes[0].plot(df1.time, df1.position_x, label='$x - x_0$')
    axes[0].plot(df1.time, df1.position_y, label='$y - y_0$')
    axes[0].plot(df1.time, df1.position_z + 8.5, label='$z - z_0$')
    axes[0].legend()
    # axes[0].set_ylabel("$q$")

    axes[1].plot(df1.time, df1.velocity_x, label='$\\dot{x}$')
    axes[1].plot(df1.time, df1.velocity_y, label='$\\dot{y}$')
    axes[1].plot(df1.time, df1.velocity_z, label='$\\dot{z}$')
    axes[1].legend()
