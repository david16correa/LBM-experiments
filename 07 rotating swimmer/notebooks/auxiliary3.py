...
def particleTrjOverview(df):
    ...
    df2 = df.query('particleId==2')

    # axes[0].plot(df.time, np.log(df.position_x)/np.log(10), label='$\\mathrm{log}_{10} x$')
    axes[0,0].plot(df1.time, df1.position_x, label='$x^1 - x^1_0$')
    axes[0,0].plot(df1.time, df1.position_y - 1.5, label='$y^1 - y^1_0$')
    axes[0,0].legend()
    # axes[0].set_ylabel("$q$")

    axes[0,1].plot(df1.time, df1.velocity_x, label='$\\dot{x}^1$')
    axes[0,1].plot(df1.time, df1.velocity_y, label='$\\dot{y}^1$')
    axes[0,1].legend()
    # axes[1].set_ylabel("$\\dot{q}$")
    
    # axes[0].plot(df.time, np.log(df.position_x)/np.log(10), label='$\\mathrm{log}_{10} x$')
    axes[1,0].plot(df2.time, df2.position_x, label='$x^2 - x^2_0$')
    axes[1,0].plot(df2.time, df2.position_y + 1.5, label='$y^2 - y^2_0$')
    axes[1,0].legend()
    axes[1,0].set_xlabel("$t$")
    # axes[0].set_ylabel("$q$")

    axes[1,1].plot(df2.time, df2.velocity_x, label='$\\dot{x}^2$')
    axes[1,1].plot(df2.time, df2.velocity_y, label='$\\dot{y}^2$')
    axes[1,1].legend()
