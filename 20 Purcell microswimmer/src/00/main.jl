...
for pair in bondPairs
    addLinearBond!(model, pair...; stiffness=10)
end
for triplet in bondTriplets
    addPolarBond!(model, triplet...; equilibriumAngle = pi, stiffness=10)
end
for Id in eachindex(hingeTriplets)
    addAuxBond!(model, hingeTriplets[Id]...; equilibriumAngle=equilibriumAngles[Id], stiffness=10)
end
